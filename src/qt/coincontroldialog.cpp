...
void CoinControlDialog::updateLabels()
{
    ...
    ui->labelTitle->setText(fSelectTransparent ?
            "Select PCOIN Outputs to Spend" :
            "Select Shielded PCOIN to Spend");
    ...
    int nDisplayUnit = BitcoinUnits::PCOIN;
                                          ...
                                          const bool isChange)
{
    ...
    // if listMode or change => show PCOIN address. In tree mode, address is not shown again for direct wallet address outputs
    if (!treeMode) {
