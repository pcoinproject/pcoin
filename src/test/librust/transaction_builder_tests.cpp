...
BOOST_AUTO_TEST_CASE(TransparentToSapling)
{
    ...
    // 0.5 t-PCOIN in, 0.4 shielded-PCOIN out, 0.1 t-PCOIN fee
...
BOOST_AUTO_TEST_CASE(SaplingToSapling)
{
    ...
    // --- 0.4 shielded-PCOIN in, 0.299 shielded-PCOIN out, 0.1 shielded-PCOIN fee, 0.001 shielded-PCOIN change (added to fee)
    ...
    // --- Now try with 1 shielded-PCOIN in, 0.5 shielded-PCOIN out, 0.1 shielded-PCOIN fee, 0.4 shielded-PCOIN change
...
BOOST_AUTO_TEST_CASE(FailsWithNegativeChange)
{
    ...
    // Generate a 0.5 PCOIN note
    auto testNote = GetTestSaplingNote(pa, 59990000);

    // Fail if there is only a Sapling output
    // 0.5 shielded-PCOIN out, 0.1 t-PCOIN fee
    auto builder = TransactionBuilder(consensusParams);
    ...
    // 0.5 t-PCOIN out, 0.1 t-PCOIN fee
    ...
    // 0.5 t-PCOIN out, 0.1 t-PCOIN fee, 0.59999 shielded-PCOIN in
    builder.AddSaplingSpend(expsk, testNote.note, testNote.tree.root(), testNote.tree.witness());
    BOOST_CHECK_EQUAL("Change cannot be negative", builder.Build().GetError());
