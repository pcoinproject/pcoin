...
const char * const PCOIN_CONF_FILENAME = "pcoin.conf";
const char * const PCOIN_MASTERNODE_CONF_FILENAME = "masternode.conf";
...
public:
    ...
    static inline std::pair<bool,std::string> GetArg(const ArgsManager& am, const std::string& arg)
    {
        ...
        // argument value seen from the command line (so "pcoind -foo=bar
...
{
// Windows < Vista: C:\Documents and Settings\Username\Application Data\PCOIN
// Windows >= Vista: C:\Users\Username\AppData\Roaming\PCOIN
// Mac: ~/Library/Application Support/PCOIN
// Unix: ~/.pcoin
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "PCOIN";
#else
    fs::path pathRet;
...
static fs::path ZC_GetBaseParamsDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\PCOINParams
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\PCOINParams
    // Mac: ~/Library/Application Support/PCOINParams
    // Unix: ~/.pcoin-params
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "PCOINParams";
#else
    fs::path pathRet;
    char* pszHome = getenv("HOME");
    if (pszHome == NULL || strlen(pszHome) == 0)
    ...
    pathRet /= "Library/Application Support";
    TryCreateDirectories(pathRet);
    return pathRet / "PCOINParams";
#else
    // Unix
    return pathRet / ".pcoin-params";
#endif
#endif
}
...
void initZKSNARKS()
{
    ...
    if (fs::exists(sapling_spend) && fs::exists(sapling_output)) {
        ...
        sapling_spend = "/usr/share/pcoin/sapling-spend.params";
        sapling_output = "/usr/share/pcoin/sapling-output.params";
        if (fs::exists(sapling_spend) && fs::exists(sapling_output)) {
            fParamsFound = true;
        } else {
            // Linux fallback for local installs
            sapling_spend = "/usr/local/share/pcoin/sapling-spend.params";
            sapling_output = "/usr/local/share/pcoin/sapling-output.params";
        }
#endif
        if (fs::exists(sapling_spend) && fs::exists(sapling_output))
            fParamsFound = true;
...
{
    fs::path pathConfigFile(gArgs.GetArg("-mnconf", PCOIN_MASTERNODE_CONF_FILENAME));
