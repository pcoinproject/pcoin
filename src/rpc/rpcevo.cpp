...
static const std::map<ProRegParam, std::string> mapParamHelp = {
        {collateralAddress,
            "%d. \"collateralAddress\"     (string, required) The PCOIN address to send the collateral to.\n"
        },
        {collateralHash,
        ...
        {ownerAddress,
            "%d. \"ownerAddress\"          (string, required) The PCOIN address to use for payee updates and proposal voting.\n"
        ...
        {payoutAddress_register,
            "%d. \"payoutAddress\"          (string, required) The PCOIN address to use for masternode reward payments.\n"
        },
        {payoutAddress_update,
            "%d. \"payoutAddress\"          (string, required) The PCOIN address to use for masternode reward payments.\n"
...
// Allows to specify PCOIN address or priv key (as strings). In case of PCOIN address, the priv key is taken from the wallet
...
static CKeyID ParsePubKeyIDFromAddress(const std::string& strAddress)
{
    ...
    if (!keyID) {
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, strprintf("invalid PCOIN address %s", strAddress));
    }
    return *keyID;
}
...
UniValue protx_update_service(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 4) {
        throw std::runtime_error(
                ...
                "\"txid\"                        (string) The transaction id.\n"
                "\nExamples:\n"
                + HelpExampleCli("protx_update_service", "\"0123456789abcdef0123456789abcdef0123456789abcdef0123456789abcdef\" \"168.192.1.100:22501\"")
        );
    }
    CheckEvoUpgradeEnforcement();
