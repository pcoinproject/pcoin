...
bool ProcessBlockFound(const std::shared_ptr<const CBlock>& pblock, CWallet& wallet, std::unique_ptr<CReserveKey>& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != g_best_block)
            return error("PCOINMiner : generated block is stale");
    }
    ...
    if (!res || sc.stateErrorFound()) {
        return error("PCOINMiner : ProcessNewBlock, block not accepted");
    }
...
{
    LogPrintf("PCOINMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    util::ThreadRename("pcoin-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        LogPrintf("Running PCOINMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void static ThreadBitcoinMiner(void* parg)
{
    ...
    try {
        ...
        LogPrintf("PCOINMiner exception");
    } catch (...) {
        LogPrintf("PCOINMiner exception");
    }

    LogPrintf("PCOINMiner exiting\n");
}
