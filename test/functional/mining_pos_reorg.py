...
class ReorgStakeTest(PcoinTestFramework):
    ...
    def check_money_supply(self, expected_pcoin):
        # verify that nodes have the expected PCOIN supply
        pcoin_supply = [self.nodes[i].getsupplyinfo(True)['transparentsupply']
                      for i in range(self.num_nodes)]
        assert_equal(pcoin_supply, [DecimalAmt(expected_pcoin)] * self.num_nodes)
    ...
    def run_test(self):
        ...
        # Verify that PCOIN supply was properly updated after the reorgs (including burned fee)
        self.log.info("Check PCOIN supply...")
