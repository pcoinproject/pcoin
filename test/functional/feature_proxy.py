...
"""Test pcoind with different proxy configuration.

Test plan:
- Start pcoind's with different proxy configurations
- Use addnode to initiate connections
- Verify that proxies are connected to, and the right connection command is given
- Proxy configurations to test on pcoind side:
...
- Create pcoinds that connect to them
- Manipulate the pcoinds using addnode (onetry) an observe effects
...
class ProxyTest(PcoinTestFramework):
    ...
    def node_test(self, node, proxies, auth, test_onion=True):
        ...
        # Note: pcoind's SOCKS5 implementation only sends atyp DOMAINNAME, even if connecting directly to IPv4/IPv6
