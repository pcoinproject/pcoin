...
if [ -n "${1:-}" ]; then
    ...
    if [[ "$OSTYPE" == "darwin"* ]]; then
        PARAMS_DIR="$HOME/Library/Application Support/PCOINParams"
    else
        PARAMS_DIR="$HOME/.pcoin-params"
    fi
fi
...
Failed to fetch the PCOIN zkSNARK parameters!
...
function main() {
    ...
    cat <<EOF
PCOIN - fetch-params.sh

This script will fetch the PCOIN zkSNARK parameters and verify their
...
EOF
    ...
    then
        ...
        cat >> "$README_PATH" <<EOF
This directory stores common PCOIN zkSNARK parameters. Note that it is
